			...
			formFields={
				<>
					...
					<MultiAutocompleteField
						...
						value={formData.workExperiences.map((workExpId) =>
							workExpOptions.find((option) => option?._id === workExpId)
						)}
						onChange={(newValue) =>
							setFormData((prevData) => ({
								...prevData,
								workExperiences: newValue.map((item) => item._id),
							}))
						}
						isOptionEqualToValue={(option, value) => option._id === value._id}
					...
					<MultiAutocompleteField
						...
						value={formData.educations.map((educationId) =>
							educationOptions.find((option) => option?._id === educationId)
						)}
						onChange={(newValue) =>
					...
					<MultiAutocompleteField
						...
						getOptionLabel={(option) => option?.title}
						value={formData.projects.map((projectId) =>
							projectOptions.find((option) => option._id === projectId)
						)}
						onChange={(newValue) =>
